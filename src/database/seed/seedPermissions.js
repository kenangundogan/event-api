...
const seedPermissions = async () => {
  try {
    ...
    // İzinleri kaynaklara göre listele
    const resources = ['user', 'event', 'category', 'system'];
    for (const resource of resources) {
      const resourcePermissions = await Permission.find({ resource, isActive: true });
      console.log(`\n${resource.toUpperCase()} kaynağı (${resourcePermissions.length} izin):`);
